...
def main():
    start=time.perf_counter(); a=run_task_a(RAW,RESULTS)
    ...
    d=run_task_d(a["data"],a["train"],RESULTS); e,kdtree=run_task_e(RESULTS)
    ...
    (RESULTS/"logs").mkdir(parents=True,exist_ok=True); (RESULTS/"logs/final_interlinked_summary.json").write_text(json.dumps(summary,indent=2),encoding="utf-8")
