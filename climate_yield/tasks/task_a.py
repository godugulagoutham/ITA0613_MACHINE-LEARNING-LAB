...
def run_task_a(raw_path,results_dir):
    tables=Path(results_dir)/"tables"; tables.mkdir(parents=True,exist_ok=True); df=clean_and_engineer(pd.read_csv(raw_path)); df.to_csv(tables/"task_a_cleaned_engineered_data.csv",index=False)
