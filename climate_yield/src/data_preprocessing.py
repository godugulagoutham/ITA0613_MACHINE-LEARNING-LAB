...
def clean_and_engineer(df):
    ...
    df["Temperature_Range"]=8.0
    df["Heat_Stress"]=(df["Temperature"]>34.0).astype(int)
    return df
...
def standardize(train,other_frames,features):
    ...
    return ((train[features]-mu)/sd).to_numpy(float),[((df[features]-mu)/sd).to_numpy(float) for df in other_frames],mu,sd
